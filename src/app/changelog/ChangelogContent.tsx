...
const ENTRIES: ChangelogEntry[] = [
  ...
  },
  {
    ...
    highlight: true,
    items: [
      { type: "fixed", text: "WCAG AA color contrast: text-tertiary colour updated from #6E6E8A to #5E5E7A — the old value failed the 4.5:1 minimum at 4.0:1 across ~470 uses site-wide. Dark mode unchanged (already passes at 5.6:1)." },
      { type: "fixed", text: "Toggle component no longer wraps a button inside a label (invalid HTML that caused screen readers to double-announce). Dark-mode toggle in the navbar now has an explicit 'Dark mode' accessible name (previously empty string)." },
      { type: "fixed", text: "ReactionBar like button now carries aria-label and aria-pressed — state changes are correctly announced to assistive technology." },
      { type: "fixed", text: "Cron-auth, PostHog webhook, and bot route secret comparisons are now all timing-safe using crypto.timingSafeEqual — prevents timing side-channel leaks consistent with the bot route fix shipped in v5.13." },
      { type: "fixed", text: "PostHog webhook Linear fetch now properly handles AbortError from the 5-second timeout and initialises the result variable before the try block — previously the AbortError could propagate unhandled and leave an uninitialised binding." },
      { type: "fixed", text: "Bot route Discord fetch gains an AbortController timeout — it was the only external call missed in the v5.13 timeout pass." },
      { type: "fixed", text: "Champions format pages now correctly say 'SP Spreads' throughout — all 8 occurrences of the legacy EV terminology replaced. The Champions format uses Stat Points (SP), not Effort Values (EVs)." },
      { type: "fixed", text: "/dashboard and /embed pages are now marked noindex/nofollow — private and iframe content should not appear in search results. Broken /champions/mega-meowstic URL removed from the sitemap." },
      { type: "new", text: "Damage calculator deep-link button added to each Pokemon card — clicking opens the Showdown damage calculator in a new tab with the Pokémon pre-populated (Mega-aware). PostHog tracks calc_opened events." },
      { type: "new", text: "FAQ page: all question headings now carry slugified id attributes for direct deep-linking and AI citation anchors. Two new FAQ entries added covering SP spreads and Regulation M-A for the Champions format." },
      { type: "improved", text: "Changelog page now has full OpenGraph and Twitter Card metadata — social shares previously fell back to the root layout defaults." },
      { type: "improved", text: "First Pokémon sprite on team overview now loads with fetchpriority=high and loading=eager (LCP optimisation) — subsequent sprites remain lazy." },
      { type: "improved", text: "TypeScript safety: DocumentWithViewTransition interface removed (TS 5.9 DOM lib includes startViewTransition natively); diff-state.ts AnyState/any replaced with Record<string,unknown> plus typed narrow-cast helpers; useShareFlow and useSlideSystem t parameter typed as TranslationKeys instead of Record<string,string>." },
      { type: "improved", text: "Removed 4 dead-code exports that had zero call sites: postBuildNotification, postToFeedbackChannel, sanitizeInput, containsInjection." },
    ],
  },
  {
    date: "May 2026",
