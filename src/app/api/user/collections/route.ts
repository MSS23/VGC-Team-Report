...
// Share IDs are 8-char alphanumeric (see src/app/api/share/[id]/route.ts).
const ShareIdSchema = z.string().regex(/^[A-Za-z0-9]{8}$/, "Invalid share ID");
// Collection IDs are generated by generateId() below (10 alphanumeric chars).
const CollectionIdSchema = z.string().regex(/^[A-Za-z0-9]{10}$/, "Invalid collection ID");
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "add-item") {
      ...
      // Verify the share is readable by this caller before adding it. Without
      // this, anyone could add a private share's id to their collection and
      // then read its full data via GET /api/user/collections/[id] (which
      // JOINs the shares table). Mirror the guard in /api/user/saved. Return
      // 404 — not 403 — so we don't confirm the existence of private shares.
      const accessible = await sql`
        ...
        LIMIT 1
      `;
      if (accessible.length === 0) return NextResponse.json({ error: "Not found" }, { status: 404 });
      await sql`
