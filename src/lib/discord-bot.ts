...
/**
 * Post a build/PR notification to the feedback channel.
 */
export async function postBuildNotification(opts: {
  issueIdentifier: string;
  issueTitle: string;
  prUrl?: string;
  status: "building" | "pr_opened" | "deployed" | "failed";
}) {
  const { channelId, configured } = getConfig();
  if (!configured) return;

  const statusMap = {
    building: { emoji: "\uD83D\uDD28", color: 0xf59e0b, label: "Building" },
    pr_opened: { emoji: "\uD83D\uDD0D", color: 0x3b82f6, label: "PR Opened" },
    deployed: { emoji: "\u2705", color: 0x10b981, label: "Deployed" },
    failed: { emoji: "\u274C", color: 0xef4444, label: "Build Failed" },
  };
  ...
  await discordFetch(`/channels/${channelId}/messages`, {
    ...
    body: JSON.stringify({
      embeds: [{
        title: `${s.emoji} ${s.label}: ${opts.issueIdentifier}`,
        description: opts.issueTitle + (opts.prUrl ? `\n\n[View PR](${opts.prUrl})` : ""),
        color: s.color,
