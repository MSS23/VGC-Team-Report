 ...
 *
 * Usage in API routes:
 *   const guard = apiGuard(request, { rateLimit: { key: "share", max: 20 } });
 ...
 */
export function apiGuard(request: Request, options: ApiGuardOptions): NextResponse | null {
  const { rateLimit, requireContentType = false, maxBodySize } = options;

  // Rate limiting
  if (rateLimit) {
    const ip = getClientIp(request);
    if (isRateLimited(`${rateLimit.key}:${ip}`, rateLimit.max, rateLimit.windowMs ?? 60_000)) {
