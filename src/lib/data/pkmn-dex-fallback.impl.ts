/**
 * Dynamic dex fallback — IMPLEMENTATION half (VGC-271).
 *
 * ⚠️  Do not import this module from anything reachable by a `"use client"`
 * component. It statically imports `dex-subset.ts` → `dex-subset.json`
 * (129,858 bytes), and a static import from the client graph is exactly what
 * put that blob back in the eager script set of `/` and `/compare`. Client
 * code goes through `pkmn-dex-fallback.ts`, which reaches this file with
 * `import()`; server code goes through `pkmn-dex-fallback.server.ts`, which
 * imports it statically and registers it synchronously.
 *
 * Coverage for every Pokemon, every form, every Mega.
...
import type { DexFallbackImpl } from "@/lib/data/pkmn-dex-fallback";
import {
  allMegaStones,
  getMegaStone,
  getSpecies,
...
// ── Base-species resolution (Species Clause) ────────────────────────────────

/**
 * Canonical base species for any forme — "Rotom-Wash" → "Rotom",
 * "Ninetales-Alola" → "Ninetales". Returns null when the subset doesn't know
 * the species; `champions-legality.ts` then falls back to its regex strip.
 */
export function getDexBaseSpecies(species: string): string | null {
  const entry = getSpecies(species);
  if (!entry) return null;
  return entry.baseSpecies ?? entry.name;
}

// ── Registration surface ────────────────────────────────────────────────────

/**
 * Bundled as one object so `pkmn-dex-fallback.ts` can install the whole
 * implementation in a single synchronous call (server) or after one dynamic
 * import (client).
 */
export const dexFallbackImpl: DexFallbackImpl = {
