...
export function TeamCardCTA({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setError("Couldn't generate the team card. Please try again in a moment.");
    } finally {
  ...
  return (
    <div className="rounded-2xl border border-border bg-gradient-to-br from-accent/10 via-accent-surface/15 to-transparent p-4 sm:p-5 flex flex-col gap-3">
      ...
      {error && (
        <div
          role="alert"
          className="text-xs sm:text-sm text-red-600 dark:text-red-400 bg-red-50/80 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-lg px-3 py-2"
        >
          {error}
        </div>
      )}
