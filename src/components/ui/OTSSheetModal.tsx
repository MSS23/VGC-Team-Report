...
export function OTSSheetModal({ pokemon, shareUrl, tournamentName, teamName, onClose }: OTSSheetModalProps) {
  ...
  return createPortal(
    ...
    >
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-background rounded-2xl shadow-2xl overflow-hidden border border-border">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border flex-shrink-0">
          <div>
            <h2 className="text-base font-bold text-text-primary">Open Team Sheet</h2>
            {(tournamentName || teamName) && (
              <p className="text-xs text-text-tertiary mt-0.5">{teamName || tournamentName}</p>
            )}
          </div>
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-lg hover:bg-surface-alt transition-colors text-text-tertiary hover:text-text-primary"
            aria-label="Close"
          >
        ...
        <div className="overflow-y-auto flex-1">
          <div ref={sheetRef} className="p-4 sm:p-5 bg-background">
            ...
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                ...
                {(teamName || tournamentName) && (
                  <p className="text-sm font-bold text-text-primary">{teamName || tournamentName}</p>
                )}
                <p className="text-[11px] text-text-tertiary mt-0.5">Species · Item · Ability · Tera Type · Moves (no EV/IV/Nature)</p>
              </div>
              {qrDataUrl && shareUrl && (
            ...
            {shareUrl && (
              <p className="text-[10px] text-text-tertiary text-center mt-3 font-mono">{shareUrl}</p>
            )}
          </div>
        </div>

        {/* Action bar */}
        <div className="flex items-center gap-2 px-5 py-3 border-t border-border flex-shrink-0 bg-surface">
          <button
            type="button"
            onClick={handleDownloadPng}
            disabled={downloading}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-accent text-white text-sm font-bold hover:brightness-110 transition-all disabled:opacity-60 disabled:cursor-wait"
          >
            {downloading ? (
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl border border-border bg-surface-alt text-sm font-semibold text-text-secondary hover:text-accent hover:border-accent/40 transition-all"
          >
